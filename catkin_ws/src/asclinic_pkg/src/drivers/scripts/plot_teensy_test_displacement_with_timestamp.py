...
class DataManager:
    def __init__(self):
        ...
        log_file = (
            self._log_dir / "log_teensy41_get_displacement_with_timestamp_080721.json"
        )
        if not log_file.exists():
            raise FileNotFoundError
        self._data = self._read_data(log_file)
        self._clean_data_index = 5
    ...
    def displacement1s(self):
        ...
        return [
            measurement["displacement1"]
            for measurement in self._data["results"][self._clean_data_index :]
        ]
    ...
    def delta_ts_between_measurements(self):
        ...
        timestamps = [
            ...
            for measurement in self._data["results"][self._clean_data_index - 1 :]
        ]
        return [y - x for x, y in zip(timestamps[:-1], timestamps[1:])]
